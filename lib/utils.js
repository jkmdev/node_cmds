...
exports.returnDirectories = function returnDirectories(contents) {
  var arr = [];
  contents.forEach(function(element) {
    if (isDirectory(element)) arr.push(element);
  });
  return arr;
}
...
function isDirectory (element) {
  ...
  return true;
  //not good way of determining if its a folder
  //return element.indexOf(".txt") != -1 ? false : true;
}
